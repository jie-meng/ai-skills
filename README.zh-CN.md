...
|---|---|---|---|
| **Meta** | | | |
| [Skill Creator](./mythril_agent_skills/skills/skill-creator/) | 为任意 AI 平台创建/优化技能和提示词，含起草、测试用例生成、评估、基准测试、描述优化。 | "帮我创建一个 Cursor skill" | — |
| **Code Review** | | | |
| [Code Review (Staged)](./mythril_agent_skills/skills/code-review-staged/) | 针对 Git 暂存区变更的上下文感知代码审查，读取相关文件验证，自动生成并复制 commit message。 | "审查暂存的代码" | `git` CLI |
| [Code Review (Local Branch Diff)](./mythril_agent_skills/skills/branch-diff-review/) | 纯本地 git 操作对比分支差异，无需平台 API，支持任意 git 仓库（GitHub、GitLab、Gitee、Bitbucket 等）。 | "review 一下 feat/123 合到 main" | `git` CLI |
| [Code Review (Github PR)](./mythril_agent_skills/skills/github-code-review-pr/) | 通过 `gh` CLI 对 Pull Request 进行上下文感知代码审查，支持 github.com 和 GitHub Enterprise（任意域名），使用 partial clone + sparse checkout 获取深层仓库上下文。 | "审查这个 PR https://github.com/xxx/yyy/pull/100" | `git` CLI, `gh` CLI |
| **Git & GitHub** | | | |
| [Git Repo Reader](./mythril_agent_skills/skills/git-repo-reader/) | 克隆、缓存并阅读任意托管平台的 git 仓库（GitHub、GitLab、Gitee、Bitbucket 等），支持跨会话复用缓存。 | "看看这个仓库 https://github.com/xxx/yyy" | `git` CLI |
| [GH Operations](./mythril_agent_skills/skills/gh-operations/) | 使用 GitHub CLI (`gh`) 执行 GitHub issue/PR 工作流：读写 issue、查看/创建 PR、添加 PR 评论（含行内 review 评论）。 | "看看这个 issue https://github.com/xxx/yyy/issues/18331" | `gh` CLI |
| **API Integrations** | | | |
| [Jira](./mythril_agent_skills/skills/jira/) | 通过 Jira REST API（内置 Python 脚本）处理 issue、sprint 和 board 工作流，无需 CLI 工具。 | "看一下这个 Jira ticket https://yourorg.atlassian.net/browse/PROJ-123" | `ATLASSIAN_API_TOKEN`, `ATLASSIAN_USER_EMAIL`, `ATLASSIAN_BASE_URL` |
| [Confluence](./mythril_agent_skills/skills/confluence/) | 通过 Confluence REST API（内置 Python 脚本）处理 page、space、comment 和 label 工作流，无需 CLI 工具。 | "看一下这个 Confluence 页面 https://yourorg.atlassian.net/wiki/spaces/XX/pages/123" | `ATLASSIAN_API_TOKEN`, `ATLASSIAN_USER_EMAIL`, `ATLASSIAN_BASE_URL` |
| [Figma](./mythril_agent_skills/skills/figma/) | 从 Figma 文件提取设计规格用于实现，涵盖布局、颜色、字体、组件规格，遇到 Figma 链接自动触发。 | "这个 Figma 设计是什么样子：https://figma.com/file/xxx" | `FIGMA_ACCESS_TOKEN` |
| **Media Processing** | | | |
| [ImageMagick](./mythril_agent_skills/skills/imagemagick/) | 通过 ImageMagick CLI 处理图片，支持缩放、格式转换、裁剪、缩略图、特效、水印、批量处理和元数据提取。 | "把 photo.jpg 缩放到 800x600" | `magick` CLI |
| [FFmpeg](./mythril_agent_skills/skills/ffmpeg/) | 通过 FFmpeg CLI 处理视频和音频，支持转码、格式转换、裁剪、合并、缩放、压缩、提取音频/字幕、GIF 创建，以及音频格式转换（MP3、WAV、PCM、OGG、AAC、FLAC、OPUS）。 | "把 video.mov 转成 mp4" | `ffmpeg` CLI |
